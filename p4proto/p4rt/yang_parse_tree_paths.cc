...
void SetUpInterfacesInterfaceConfigPipelinename(const char *pipeline_name,
  ...
  auto on_set_functor =
    ...
    request->set_node_id(node_id);
    request->set_port_id(port_id);
    request->SetRequest::Request::Port::mutable_pipeline_name()->PipelineNameConfigured::set_pipeline_name((const char*)pipeline_name.c_str());
...
//
void SetUpInterfacesInterfaceConfigPcibdf(const char *pci_bdf,
                                          uint64 node_id,
                                          uint64 port_id,
                                          TreeNode* node,
                                          YangParseTree* tree) {
